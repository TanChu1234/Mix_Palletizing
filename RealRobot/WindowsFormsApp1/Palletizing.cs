...
namespace WindowsFormsApp1
{
    public partial class Palletizing : Form
    {
        ...
        private void Solve_Click(object sender, EventArgs e)
        {
            ...
            dynamic py = engine.ExecuteFile(@"D:\ABB Docs\build\test2.py");
            ...
            if (l2 != null)
            {
                ListViewItem item = new ListViewItem("AdminSet");
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked == true)
            {
                for (int i = 0; i < Convert.ToInt64(l1[-8]); i++)   // ListPosOnPallet[-1] TOTAL BOXES THAT FIT 
                {
                    ...
                    Num numBox;
                    //Num config = new Num();
                    RapidData rd_targets = controller.Rapid.GetTask("T_ROB1").GetModule("Wizard").GetRapidData("tgPos");
                    RapidData offset = controller.Rapid.GetTask("T_ROB1").GetModule("Wizard").GetRapidData("offset");
                    RapidData No = controller.Rapid.GetTask("T_ROB1").GetModule("Wizard").GetRapidData("no");
                    RapidData fittedBox = controller.Rapid.GetTask("T_ROB1").GetModule("Wizard").GetRapidData("fittedBox");
                    //RapidData Config = controller.Rapid.GetTask("T_ROB1").GetModule("Wizard").GetRapidData("config");
                    ...
                    off.Z = Convert.ToInt64(l1[i][3]) + 300;
            ...
            else if (radioButton2.Checked == true)
            {
                for (int i = 0; i < Convert.ToInt64(l2[-8]); i++)   // ListPosOnPallet[-1] TOTAL BOXES THAT FIT 
                {
                    ...
                    off.Z = Convert.ToInt64(l2[i][3]) + 300;
            ...
            else if (radioButton3.Checked == true)
            {
                for (int i = 0; i < Convert.ToInt64(l3[-8]); i++)   // ListPosOnPallet[-1] TOTAL BOXES THAT FIT 
                {
                    ...
                    off.Z = Convert.ToInt64(l3[i][3]) + 300;
            ...
            else if (radioButton4.Checked == true)
            {
                for (int i = 0; i < Convert.ToInt64(l4[-8]); i++)   // ListPosOnPallet[-1] TOTAL BOXES THAT FIT 
                {
                    ...
                    off.Z = Convert.ToInt64(l4[i][3]) + 300;
        ...
        private void Díplay_Click(object sender, EventArgs e)
        {
            var engine = Python.CreateEngine();
            var searchPaths = engine.GetSearchPaths();
            searchPaths.Add(@"C:\Users\USER\.vscode\extensions\ms-python.python-2022.4.1\pythonFiles\lib\jedilsp\jedi\third_party\typeshed\stdlib\3\random.pyi"); // add your path to the stdlib here
